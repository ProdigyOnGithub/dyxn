...
# We assume ANON user since auth was removed
ANON_OWNER_ID = 0
...
def list_sessions(db: Session = Depends(get_db)):
    ...
    sessions = db.query(ChatSession).filter(ChatSession.user_id == ANON_OWNER_ID).order_by(ChatSession.created_at.desc()).all()
    return [{"id": s.id, "title": s.title} for s in sessions]

@router.delete("/sessions/{session_id}")
def delete_session(session_id: str, db: Session = Depends(get_db)):
    """Deletes a chat session."""
    session_obj = db.query(ChatSession).filter(ChatSession.id == session_id, ChatSession.user_id == ANON_OWNER_ID).first()
    if not session_obj:
...
def chat(session_id: str, chat_message: ChatMessage, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
